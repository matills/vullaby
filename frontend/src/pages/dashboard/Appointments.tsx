...
const Appointments = () => {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery({
    queryKey: ["appointmentStats", businessId],
    queryFn: async () => {
      const response = await fetch(`/api/appointments/stats?businessId=${businessId}`);
  ...
  const { data: upcomingAppointments, isLoading: appointmentsLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/appointments/upcoming?businessId=${businessId}&limit=10`);
